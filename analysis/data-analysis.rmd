...
result_path <- "D:/MESTRADO/repo/faas-simulator/results_bigsample"
techs <- c('RequestHedgingOpt', 'RequestHedgingDefault', 'Vanilla')
probs      <- c('tlprobp90', 'tlprobp95', 'tlprobp99')
idletimes  <- c('idletimeINF', 'idletime0.0', 'idletime5.0', 'idletime15.0')
hasOracle <- c('hasOraclefalse') #c('hasOraclefalse', 'hasOracletrue')
...
df_for_invocations <- data.frame(technique, tailLatencyProb, idletime, p50, p90, p95, p99)
...
plot_results(df_for_invocations, df_for_invocations$p50, "response time (ms)", "50th of invocations' response time for each idletime")
plot_results(df_for_invocations, df_for_invocations$p90, "response time (ms)", "90th of invocations' response time for each idletime")
plot_results(df_for_invocations, df_for_invocations$p95, "response time (ms)", "95th of invocations' response time for each idletime")
plot_results(df_for_invocations, df_for_invocations$p99, "response time (ms)", "99th of invocations' response time for each idletime")

plot_results(df_for_replicas, df_for_replicas$uptime/1000/60/60, "total uptime (hour)", "Total replicas UpTime for each idletime")
plot_results(df_for_replicas[df_for_replicas$idletime != Inf, ], df_for_replicas[df_for_replicas$idletime != Inf, ]$uptime/1000/60/60, "total uptime (hour)", "Total replicas UpTime for each idletime (excluding Inf)")

plot_results(df_for_replicas, df_for_replicas$busyTime/1000/60/60, "total busy-time (hour)", "Total replicas BusyTime for each idletime")
plot_results(df_for_replicas, df_for_replicas$replicasCount, "number of replicas", "Total number of replicas created for each idletime")
```
