...
inv2021_merged = select(inv2021_merged[order(inv2021_merged$start_timestamp),], app, func, start_timestamp, duration, end_timestamp, p95, p100,is_tail_latency)

inv2021_merged_nozero = subset(inv2021_merged, duration != 0)

nrow(inv2021_merged_nozero) / nrow(inv2021_merged)
...
```{r}
invoc_vanilla              = read.csv("C:/Users/david/OneDrive/Documentos/GitHub/faas-simulator/vanilla-invocations.csv", header = TRUE)
invoc_requestHedgedDefault = read.csv("C:/Users/david/OneDrive/Documentos/GitHub/faas-simulator/RequestHedgingDefault-invocations.csv", header = TRUE)
invoc_requestHedgedDefault <- transform(invoc_requestHedgedDefault, responseTimeFinal = pmin(responseTime, rHresponseTime))

rep_requestHedgedDefault   = read.csv("C:/Users/david/OneDrive/Documentos/GitHub/faas-simulator/RequestHedgingDefault-replicas.csv", header = TRUE)
rep_vanilla                = read.csv("C:/Users/david/OneDrive/Documentos/GitHub/faas-simulator/vanilla-replicas.csv", header = TRUE)

quantile(invoc_vanilla$responseTime, probs = 0.95)
quantile(invoc_requestHedgedDefault$responseTimeFinal, probs = 0.95)


hist(invoc_vanilla$responseTime)
hist(invoc_requestHedgedDefault$responseTimeFinal)
