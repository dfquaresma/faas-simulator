...
calculate_percentiles <- function(x) {
  if (length(x) <= 1) {
    return(c(p90 = x[1], p95 = x[1], p99 = x[1], p100 = x[1]))
  }
  ...
  return(c(p90 = p90, p95 = p95, p99 = p99, p100 = p100))
}
...
```{r}
inv2021 = read.csv("D:/MESTRADO/repo/faas-simulator/azure/AzureFunctionsInvocationTraceForTwoWeeksJan2021.csv", header = TRUE)
...
inv2021_split_filtered <- lapply(inv2021_split_nonull, add_percentile_partials)
...
inv2021_merged = select(inv2021_merged[order(inv2021_merged$start_timestamp),], app, func, start_timestamp, duration, end_timestamp, p90, p95, p99, p100, partials_p90, partials_p95, partials_p99, partials_p100) #,is_tail_latency)

# save processed data
write.csv(inv2021_merged, file = "D:/MESTRADO/repo/faas-simulator/azure/AzureFunctionsInvocationTraceForTwoWeeksJan2021-processed.csv", row.names = FALSE)
```
